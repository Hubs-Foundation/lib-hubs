...
export class ParticleEmitter extends Mesh {
  ...
  update(dt: number) {
    const position = this.geometry.attributes.particlePosition.array as Float32Array;
    const color = this.geometry.attributes.particleColor.array as Float32Array;
    const particleAngle = this.geometry.attributes.particleAngle.array as Float32Array;
    ...
    for (let i = 0; i < this.particleCount; i++) {
      ...
      const velFactor = EasingFunctions[this.velocityCurve](normalizedAge);
      const sizeFactor = EasingFunctions[this.sizeCurve](normalizedAge);
      const colorFactor = EasingFunctions[this.colorCurve](normalizedAge);
